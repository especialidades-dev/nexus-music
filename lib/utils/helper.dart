...
void printERROR(dynamic text, {String tag = "Nexus Music"}) {
  if (kReleaseMode) return;
  debugPrint("\x1B[31m[$tag]: $text\x1B[0m");
}

void printWarning(dynamic text, {String tag = 'Nexus Music'}) {
  if (kReleaseMode) return;
  debugPrint("\x1B[33m[$tag]: $text\x1B[34m");
}

void printINFO(dynamic text, {String tag = 'Nexus Music'}) {
  if (kReleaseMode) return;
...
Future<bool> newVersionCheck(String currentVersion) async {
  try {
    final tags = (await Dio()
            .get("https://api.github.com/repos/especialidades-dev/nexus-music/tags"))
        .data;
